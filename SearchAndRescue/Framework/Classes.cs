    ...
    public class Classes
    {
        ...
        public class AddRecoveryContractRequest
        {
            ...
            public void AddRecoveryContract()
            {
                ...
                var oldEmployers = (List<string>)_fieldSetContractEmployers.GetValue(StarSystemSystem.Def);
                var oldTargets = (List<string>)_fieldSetContractTargets.GetValue(StarSystemSystem.Def);
                ...
                var difficultyRange = _getContractRangeDifficultyRange.Invoke(Sim, new object[] { StarSystemSystem, Sim.SimGameMode, Sim.GlobalDifficulty });
                ...
                var potentialContractsByType = (Dictionary<int, List<ContractOverride>>)_getContractOverrides.Invoke(Sim, new object[] { difficultyRange, validTypes });
                ...
                var validParticipants = _getValidParticipants.Invoke(Sim, new object[] { StarSystemSystem });

                if (!(bool)_hasValidMaps.Invoke(Sim, new object[] { StarSystemSystem, playableMaps }))
                {
                    ModInit.modLog.Debug?.Write($"AddRecoveryContract - false _hasValidMaps");
                    return;
                }
                else if (!(bool)_hasValidContracts.Invoke(Sim, new object[] { difficultyRange, filteredContractsByType }))
                {
                    ModInit.modLog.Debug?.Write($"AddRecoveryContract - false _hasValidContracts");
                    return;
                }
                else if (!(bool)_hasValidParticipants.Invoke(Sim, new object[] { StarSystemSystem, validParticipants }))
                {
                ...
                var MapEncounterContractData = _fillMapEncounterContractData.Invoke(Sim, new object[] { StarSystemSystem, difficultyRange, filteredContractsByType, validParticipants, level });
                ...
                while (!HasContracts && activeMaps.ActiveListCount > 0)
                {
                    ...
                    MapEncounterContractData = _fillMapEncounterContractData.Invoke(Sim, new object[] { StarSystemSystem, difficultyRange, filteredContractsByType, validParticipants, level });
                }
                ...
                Contract contract = (Contract)_createProceduralContract.Invoke(Sim, new object[] { StarSystemSystem, true, level, MapEncounterContractData, gameContext });

                // Restore system and faction to previous values, now that we've forced the game to generate our desired contract.
                _fieldSetContractEmployers.SetValue(StarSystemSystem, oldEmployers);
                _fieldSetContractTargets.SetValue(StarSystemSystem, oldTargets);
        ...
        public class MissingPilotInfo
        {
            ...
            public MissingPilotInfo(PilotDef missingPilotDef, string pilotSimUID, string missingPilotSystem, Biome.BIOMESKIN pilotBiomeSkin)
            {
